...
def simulate_for_age(age_months, num_runs=20):
    ...
    # Run simulations
    # Using a smaller number of runs per point to keep execution time reasonable
...
def generate_stacked_area_chart():
    months_points = list(range(0, 61, 10)) # 0, 10, 20, 30, 40, 50, 60
    ...
    print(f"Starting simulation for ages: {months_points} months")
    print("This may take a while...")
    
    for m in tqdm(months_points):
        # Using 15 runs per point for balance of speed and smoothness
        results = simulate_for_age(m, num_runs=15)
        for s in data.keys():
    ...
    # Custom colors (avoiding green at top)
    # Standby (Bottom) -> Nav (Top)
    # Colors: Grey/Blue -> Orange -> Red -> Purple -> Cyan
    colors = [
        '#A0A0A0', # Standby - Grey
        '#FFB347', # Social - Pastel Orange
        '#FF6961', # Game - Pastel Red
        '#B39EB5', # Video - Pastel Purple
        '#77DD77'  # Nav - Pastel Green (Wait, user said NO GREEN AT TOP)
    ]
    
    # Revised Colors to satisfy "No Green at Top"
    colors = [
    ...
    plt.title('Energy Consumption Composition vs. Battery Aging', fontsize=16, pad=20, weight='bold')
    plt.xlabel('Battery Age (Months)', fontsize=12)
    plt.ylabel('Total Energy Consumed per Cycle (Wh)', fontsize=12)
    plt.xlim(0, 60)
    
    # Legend
    plt.legend(loc='upper right', frameon=True, fontsize=10, bbox_to_anchor=(1.15, 1))
    ...
    plt.text(62, total_end, f"-{drop_pct:.1f}% Capacity", 
             verticalalignment='center', color='red', fontsize=10, weight='bold')

    plt.tight_layout()
    output_path = os.path.join(current_dir, 'aging_energy_stacked_area.png')
